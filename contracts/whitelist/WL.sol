...
contract WL is OwnableUpgradeable {
  address private dai;
  ...
  function initialize(address _dai, address _aDiat, address _diatomDao) public initializer {
    dai = _dai;
    aDiat = _aDiat;
  ...
  function addapprovedBuyers(address[] calldata buyers) external onlyOwner {
    for (uint256 i; i < buyers.length; i++) {
      // approve buyers and set their limit to 80
